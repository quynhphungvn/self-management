function createNewWordRef(subtitle) {
	const requestInfo = {
...
function fetchKnownWordCallback(listKnownWordText, subtitle) {
	let subArray = subtitle.replace(/\d\d:\d\d\n/g, "").replace(/\r?\n|\r/g, " ").split(" ");	
	let wordHtml = "";
	let listKnownWord = JSON.parse(listKnownWordText).data;
...
function addToWordKnownCallback(text, word) {
	let mes = JSON.parse(text);
	if (mes.status == Status.ADD_SUCCESS)
